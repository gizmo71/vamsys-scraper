...
def get_airline_map(driver):
    ...
    #print('snapshot', airline_snapshot);
    data = json.loads(airline_snapshot)['data']
    return dict((row[0]['pilot_username'], row[0]) for row in data['starredAirlines'][0] + data['nonStarredAirlines'][0])

id_to_airline = get_airline_map(driver)
#print('map', id_to_airline)
#raise ValueError('grolsch')
